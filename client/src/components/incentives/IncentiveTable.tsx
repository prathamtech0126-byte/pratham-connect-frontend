...
import { Search, X, ChevronDown, SlidersHorizontal, ArrowLeftRight } from 'lucide-react'
        ...
        <table className="w-full text-sm border-collapse">
          <thead>
            ...
            <tr className="bg-muted/50 border-b border-border">
              ...
              <th className="px-4 py-3 text-center text-[11px] font-semibold text-muted-foreground uppercase tracking-wider min-w-[130px]">Eligibility</th>
              ...
              <th className="px-4 py-3 text-center text-[11px] font-semibold text-muted-foreground uppercase tracking-wider min-w-[120px]">Status</th>
                  ...
                  >
                    ...
                    <td className="px-4 py-3">
                      <span
                        ...
                        className={cn(
                          'inline-block max-w-[min(180px,40vw)] truncate px-2.5 py-0.5 rounded-full text-[11px] font-semibold tracking-wide align-middle',
                          (row.saleTypeName?.trim() || (row.saleTypeIsNull && (row.otherProducts?.items?.length ?? 0) > 0)) ? 'normal-case' : 'uppercase',
                          saleBadgeClass[
                            (row.saleTypeIsNull && (row.otherProducts?.items?.length ?? 0) > 0)
                              ? 'other_products'
                              : row.saleType
