...
export function VisitorRulesTab({ rules }: Props) {
  ...
  const [schengenDraft, setSchengenDraft] = useState<CategoryDraft[]>([])
  const [dirty, setDirty] = useState(false)

  useEffect(() => {
    setCoreDraft((rules.coreVisitorRules ?? []).map(toDisplay))
    setProductDraft((rules.visitorProductRules ?? []).map(toDisplay))
    setSchengenDraft((rules.schengenVisaRules ?? []).map(toDisplay))
    setDirty(false)
  }, [rules])

  const coreError = validateCategories(coreDraft)
  const productError = validateCategories(productDraft)
  const schengenError = validateCategories(schengenDraft)
  const hasError = !!coreError || !!productError || !!schengenError
  ...
  const saveMutation = useMutation({
    mutationFn: () =>
      saveVisitorRules({
        ...
        schengenVisaRules: schengenDraft.map(toDomain),
      }),
    onSuccess: () => {
  ...
  const mark = (fn: (d: CategoryDraft[]) => void) => (updated: CategoryDraft[]) => {
    fn(updated)
    setDirty(true)
  }
    ...
    <div className="p-4 space-y-4">
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
        <CategorySection
          title="Core Visitor"
          draft={coreDraft}
          onChange={mark(setCoreDraft)}
        />
        <CategorySection
          title="Visitor Product Core"
          draft={productDraft}
          onChange={mark(setProductDraft)}
        />
        <CategorySection
          title="Schengen Visa"
          draft={schengenDraft}
          onChange={mark(setSchengenDraft)}
        />
      </div>
