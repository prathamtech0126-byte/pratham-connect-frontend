...
import logoLight from "@/assets/images/pratham-logo-light-mode.svg";
import { ModeToggle } from "@/components/mode-toggle";
...
export default function Login() {
    ...
    const handleLogin = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if (error.message?.includes('CORS') || error.code === 'ERR_CORS') {
                msg = "CORS Error: Backend server needs to allow requests from this domain.";
            } else if (error.code === "ECONNABORTED" || error.code === "ETIMEDOUT") {
                msg = "Request timeout. The server might be waking up. Please try again in a moment.";
            } else if (error.code === "ERR_NETWORK" || error.code === "ECONNREFUSED") {
                msg = "Cannot reach server. The server may be spinning up. Please wait and try again.";
            } else if (error.response) {
                if (error.response.status === 0) {
                    msg = "CORS Error: Backend is blocking requests from this domain.";
                } else if (error.response.status === 500) {
...
//             // Handle remember me: store email if checked
//             if (rememberMe) {
...
//             {/* Main Card Container - Increased width from max-w-[400px] to max-w-[480px] */}
//             <div className="w-full max-w-[480px] px-4 relative z-10">
//                 <Card className="border-0 shadow-lg bg-white dark:bg-gray-900 rounded-2xl">
//                     <CardContent className="pt-10 pb-10 px-8">
//                         <div className="space-y-6">
...
//                                 {/* Remember Me Row - Simple checkbox + optional forgot link */}
//                                 <div className="flex items-center justify-between">
...
//                                             className="w-4 h-4 rounded border-gray-300 dark:border-gray-600 text-gray-700 focus:ring-gray-400"
//                                         />
//                                         <span className="text-sm text-gray-600 dark:text-gray-400">
//                                             Remember me
//                                         </span>
//                                     </label>
//                                     {/* Optional: Add a simple forgot password link if needed */}
//                                     <button
//                                         type="button"
//                                         onClick={() => {
//                                             toast({
